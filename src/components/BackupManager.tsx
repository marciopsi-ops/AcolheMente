...
export const BackupManager: React.FC<BackupManagerProps> = ({
  ...
  return (
    <div className="w-full flex flex-col gap-5">
      {/* Header Banner */}
      <div className="bg-white p-4 sm:p-5 rounded-2xl shadow-xs border border-soft flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-forest/5 flex items-center justify-center text-forest border border-soft shrink-0">
            <Database className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg font-serif font-bold text-forest">
                Backup & Restauração de Dados
              </h2>
              <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase bg-emerald-100 text-emerald-800 border border-emerald-200 flex items-center gap-1">
                <ShieldCheck className="w-3 h-3" /> Seguro
              </span>
            </div>
            <p className="text-[11px] text-forest/70 mt-0.5 max-w-2xl leading-relaxed">
              Exporte todos os cadastros clínicos, psicólogos, triagens, mensagens e configurações em formato estruturado.
            </p>
          </div>
        </div>

        {lastExportTime && (
          <div className="text-[11px] text-forest/60 bg-warm px-3 py-1.5 rounded-lg border border-soft flex items-center gap-1.5 shrink-0">
            <Calendar className="w-3.5 h-3.5 text-forest/40" />
            <span>Último backup: <strong>{lastExportTime}</strong></span>
          </div>
        )}
      </div>
